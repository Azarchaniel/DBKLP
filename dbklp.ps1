...
if ($env:computername -eq "DESKTOP-KGPMJ57" -or ($env:computername -eq "DESKTOP-8S0PLJ1" -and $env:username -eq "zanet")) #if it's gf's PC (or my computer but her acount), use these variables
{
 ...
 elseif ($env:computername -eq "DESKTOP-AFQ4MFR" -and ($env:username -eq "LubosCs")) #if it's mine pc
{
...
if ((Test-Path @($GdriveS, $GdriveZ, $GdriveD)) -contains $false) {
    Write-Host "Nie je zložka Google Drive!"
...
try {
    ...
    #if file in Backup is older than 30 days, delete it
    #if (Test-Path $GdriveZ) {Get-ChildItem -Path $GdriveZ | Where-Object {($_.LastWriteTime -lt (Get-Date).AddDays(-30))} | Remove-Item | Write-Host 'Vymazaný súbor starší než 30 dní.' }

    #if there's more than 10 files, delete the rest
    if ((Get-ChildItem $GdriveZ | Measure-Object ).Count -igt 10) {
        Write-Host "Viac nez desat suborov"
        Get-ChildItem $GdriveZ | sort LastWriteTime | Select -first ((Get-ChildItem $GdriveZ | Measure-Object ).Count - 10) | Remove-Item
    }
} catch [Microsoft.PowerShell.Commands.MoveItemCommand]{
    Write-Host "Chyba pri presune súboru."
    Remove-Item -Path (Join-Path -Path $GdriveZ -ChildPath "DBKLP$(get-date -f yyMMdd).xlsm")
    Remove-Item -Path $GdriveS
    #D:\Programy\dbklp.ps1      #call function recursively
